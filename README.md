...
使用 Sora2 API 生成视频。支持文生视频（txt2vid）和图生视频（img2vid）两种模式。工具会自动等待视频生成完成并下载到指定目录。

**参数：**

- `prompt` (必需): 视频的文本描述（支持中英文）
- `images` (可选): 源图像数组，用于图生视频。可以是单个图像或图像数组。每个图像可以是 URL（http/https）或本地文件路径（必须是绝对路径）。本地图像会自动转换为 base64。如果不提供，将生成纯文本生成的视频
- `model` (可选): 使用的模型（默认: `sora-2`）
- `aspect_ratio` (可选): 视频宽高比 - 可选值: `16:9`, `9:16`, `1:1`（默认: `16:9`）
- `hd` (可选): 生成 HD 高清质量（会增加约 8 分钟生成时间，默认: false）
- `duration` (可选): 视频时长（秒）- 可选值: `10` 或 `15`（默认: `10`）
- `watermark` (可选): 是否包含水印（默认: false）
- `private` (可选): 保持视频私密（默认: true）
- `output_directory` (可选): 保存生成视频的目录（必须是绝对路径）。如果不指定，视频不会被下载。如果设置为空字符串，视频将保存到默认临时目录
- `filename` (可选): 自定义保存的文件名（默认: sora2_{timestamp}.mp4）。如果未提供扩展名，会自动添加 .mp4
...
查询 Sora2 视频生成任务的状态。此工具用于手动检查任务进度。

**参数：**

- `task_id` (必需): 由 `generate_sora2_video` 返回的任务 ID

**注意：** `generate_sora2_video` 工具已经会自动等待完成，所以此工具主要用于手动查询。
...
- `API_KEY` (必需): 您的 API 密钥（同时用于图像生成和 Sora2 视频生成）
- `API_ENDPOINT` (可选): API 端点 URL，默认为 `http://localhost:8000/v1/chat/completions`
- `MODEL_NAME` (可选): 模型名称，默认为 `gemini-2.5-flash-image`
- `SORA2_API_ENDPOINT` (可选): Sora2 API 端点 URL，默认为 `https://gpt-best.apifox.cn`
- `SORA2_API_KEY` (可选): Sora2 API 密钥，如果不设置则使用 `API_KEY`
...
#### 生成视频（文生视频）
```
生成一个 10 秒的视频：一只可爱的小猫在花园里玩耍
```

#### 从图片生成视频（图生视频 - 使用 URL）
```
将这张图片转换为视频：https://example.com/image.jpg，内容：让画面中的人物动起来
```

#### 从本地图片生成视频（图生视频 - 使用本地文件）
```
将本地图片转换为视频：/Users/username/Pictures/photo.jpg，内容：让照片动起来，人物微笑挥手
```

#### 生成高清视频
```
生成一个 15 秒的高清视频：未来城市的日落景象
```
